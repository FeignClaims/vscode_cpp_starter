...
// note: 利用 RAII 技术，提供一个表示 Vector 内存的辅助类来构造类
...
// note: P331 赋值操作：比较直接的实现，提供强保障（见 13.2 节）
template <typename T, typename A>
auto Vector<T, A>::operator=(const Vector& a) -> Vector& {
  VectorBase<T, A> b{a.vb_.alloc, a.size()};            // 获取内存空间
  std::uninitialized_copy(a.begin(), a.end(), b.elem);  // 拷贝元素
